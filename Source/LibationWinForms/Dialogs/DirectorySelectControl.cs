...
public partial class DirectorySelectControl : UserControl
{
	...
	internal string AddSubDirectoryToPath(string? path)
		=> string.IsNullOrWhiteSpace(_subDirectory)
		? string.IsNullOrWhiteSpace(path) ? "" : path
		: string.IsNullOrWhiteSpace(path) ? _subDirectory : System.IO.Path.Combine(path, _subDirectory);
	...
	private DirectoryComboBoxItem? selectedItem => directoryComboBox.SelectedItem as DirectoryComboBoxItem;
