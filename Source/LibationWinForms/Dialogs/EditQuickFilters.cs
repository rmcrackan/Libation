...
public partial class EditQuickFilters : Form
{
	...
	internal class DisableButtonCell : AccessibleDataGridViewButtonCell
	{
		private int LastRowIndex
			=> DataGridView?.Rows.Count is null or 0 ? 0
			: DataGridView.Rows[^1].IsNewRow ? DataGridView.Rows[^1].Index - 1
			: DataGridView.Rows[^1].Index;
		...
		protected override void Paint(Graphics graphics, Rectangle clipBounds, Rectangle cellBounds, int rowIndex, DataGridViewElementStates elementState, object? value, object? formattedValue, string? errorText, DataGridViewCellStyle cellStyle, DataGridViewAdvancedBorderStyle advancedBorderStyle, DataGridViewPaintParts paintParts)
		{
			...
			var isNewRow = OwningRow?.IsNewRow ?? false;
	...
	private void saveBtn_Click(object sender, EventArgs e)
	{
		var list = dataGridView1.Rows
			...
			.Select(r => new { Filter = r.Cells[COL_Filter].Value?.ToString(), FilterName = r.Cells[COL_FilterName].Value?.ToString() })
			.Select(r => r.Filter is null ? null : new QuickFilters.NamedFilter(r.Filter, r.FilterName))
			.OfType<QuickFilters.NamedFilter>()
			.ToArray();
