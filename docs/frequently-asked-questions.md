...
Audible's early September 2026 changes can cause license refusals, including `CustomerThrottled`, for existing device registrations. Try the [full registration recovery procedure](/docs/advanced/device-registration#how-to-register-an-account-again): deregister Amazon devices named "Libation", upgrade to the [latest Libation](https://github.com/rmcrackan/Libation/releases/latest), record its registration region and additional marketplaces, remove and save the affected account, close and reopen Libation, verify it is gone, then re-add it. Follow the procedure to sign in and restore all recorded marketplaces before the full scan and download retry. Removing the account preserves your library and downloaded files. We recommend the full sequence, although we cannot be certain every step is necessary.
