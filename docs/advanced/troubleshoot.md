...
1. **Device-registration trouble after Audible's early September 2026 changes** -- for `CustomerThrottled`, or when Audible's app plays the title, try the [full recovery procedure](/docs/advanced/device-registration#how-to-register-an-account-again): deregister Libation devices, upgrade to the [latest release](https://github.com/rmcrackan/Libation/releases/latest), record the registration region and additional marketplaces, remove/save the account, restart, verify removal, and re-add. Follow the procedure to sign in and restore all recorded marketplaces before the full scan and download retry. Your library and files are preserved. This is a recommended workaround, not a guaranteed fix.
2. **Temporary Audible outage or Plus throttling** -- if recovery does not help, wait 24 to 48 hours, sometimes a few days. Heavy Plus downloading can still trigger genuine rate limits.
